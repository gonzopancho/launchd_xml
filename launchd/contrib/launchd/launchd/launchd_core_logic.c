...
/* introduced in launchd 12.22.2005 */
/* this is to get rid of error in uninitialized root_bootstrap */
/* jmp 01.18.2006 */
...
simple_zombie_reaper(void *obj __attribute__((unused)), struct kevent *kev)
{
	...
	/* jmp - need to decrement total_children */
	total_children--;
}
...
job_import(launch_data_t pload)
{
	...
	if ((tmp = launch_data_dict_lookup(pload, LAUNCH_JOBKEY_STARTINTERVAL))) {
		...
		/*
		 * At the time of writing, milliseconds is the only 
		 *  recognized unit for EVFILT_TIMER.
		 * Note: Replacing NOTE_SECONDS with 0 to avoid 
		 *         confusion.
		 *       Added multiplier to get milliseconds(1000).
		 */
		if (j->start_interval == 0)
			job_log(j, LOG_WARNING, "StartInterval is zero, ignoring");
		else if (-1 == kevent_mod((uintptr_t)&j->start_interval, EVFILT_TIMER, EV_ADD, 0, 
					  (intptr_t)(j->start_interval * 1000), &j->kqjob_callback))
...
static void dump_tm(struct tm *when)
{
	char *str;
	asprintf(&str, "\nseconds: %d\nminutes: %d\nhour: %d\n" \
		 "m-day: %d\nmonth: %d\nyear: %d\nw-day: %d",
		 when->tm_sec, when->tm_min, when->tm_hour,
		 when->tm_mday, when->tm_mon, when->tm_year,
		 when->tm_wday);
	log_info("%s", str);
}
...
calendarinterval_setalarm(struct jobcb *j, struct calendarinterval *ci)
{
	...
	if (-1 != ci->when.tm_wday && ci->when.tm_mday == -1) {
		int delta, realwday = ci->when.tm_wday;

		log_info("computing weekday: %d", ci->when.tm_wday);
	...
	/*
	 * Increment the next larger unit if the target is lesser than
	 *  the current value.
	 */
#if 0 /* replaced with a more cron-like behavior */
	...
	if (-1 != ci->when.tm_mon && latertm.tm_mon < nowtm->tm_mon) {
		latertm.tm_year++;
	} else if (-1 != ci->when.tm_mon && latertm.tm_mon > nowtm->tm_mon) {
		/* done */
	} else {
		if ((-1 != ci->when.tm_mday) && (latertm.tm_mday < nowtm->tm_mday)) {
			latertm.tm_mon++;
		} else if ((-1 != ci->when.tm_mday) && (latertm.tm_mday > nowtm->tm_mday)) {
			/* done */
		} else {
			if (-1 != ci->when.tm_hour && latertm.tm_hour < nowtm->tm_hour) {
				latertm.tm_mday++;
			} else if (-1 != ci->when.tm_hour && latertm.tm_hour > nowtm->tm_hour) {
				/* done */
			} else {
				if (-1 != ci->when.tm_hour && latertm.tm_hour < nowtm->tm_hour) {
					...
					if (-1 != ci->when.tm_min && latertm.tm_min < nowtm->tm_min) {
						latertm.tm_hour++;
					} else if (-1 != ci->when.tm_min && latertm.tm_min > nowtm->tm_min) {
						/* done */
					} else {
						/* add one minute to ensure execution */
						match=1;
					}
				}
			}
	...
	if (match) {
		if (ci->when.tm_min == -1) {
			latertm.tm_min++;
		} else if (ci->when.tm_hour == -1) {
			latertm.tm_hour++;
		} else if (ci->when.tm_mday == -1) {
			latertm.tm_mday++;
		} else if (ci->when.tm_mon == -1) {
			latertm.tm_mon++;
		} else {
			latertm.tm_year++;
		}
	...
	/* get time from now */
	later = (later - mktime(localtime(&now))) * 1000;
	log_info("set alarm after: %ld seconds", later/1000);

	/*
	 * FreeBSD kevent does not support NOTE_SECONDS.
	 *  - use a multiplier to get milliseconds.
	 */
	if (-1 == kevent_mod((uintptr_t)ci, EVFILT_TIMER, EV_ADD, 0, later, j)) {
...
#define DEF_LBLSZ   1024

void init_root_job()
{
	char bslabel[DEF_LBLSZ] = "100000";
	struct jobcb *j;

	j = calloc(1, sizeof(struct jobcb) + sizeof(bslabel) + 1);

	if (launchd_assumes(j!= NULL)) {
		fprintf(stderr, "Error allocating job failed!");
		return;
	}

	strncpy(j->label, bslabel, sizeof(bslabel));
	j->kqjob_callback = job_callback;
	j->bstrap = NULL;
